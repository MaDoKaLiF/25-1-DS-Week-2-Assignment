...
def get_dataloader_escape(args, tokenizer, rank, world_size):
    dataset, dataset_train, dataset_test = None, None, None
    jsonl_file_path = f"../STaR/{args.task}/{args.task}_irpo_{args.seed}/gsm8k_irpo_{args.seed}_{args.exp_iter-2}/irpo_query_acc_{args.exp_iter-1}.jsonl"
    jsonl_data = load_jsonl(jsonl_file_path)
    questions_to_remove = [entry['question'] for entry in jsonl_data]
    if (args.task == "gsm8k"):
        if args.exp_iter==2:
            dataset = load_from_disk("../datasets/data_gsm8k")
            dataset.save_to_disk(f"../datasets/data_gsm8k_escape_1")

        dataset = load_from_disk(f"../datasets/data_gsm8k_escape_{args.exp_iter-1}")
        filtered_dataset = remove_matching_questions(dataset, questions_to_remove)
        filtered_dataset.save_to_disk(f"../datasets/data_gsm8k_escape_{args.exp_iter}")
        dataset_train = dataset["train"].map(lambda examples: preprocess_function(args, examples, tokenizer, "train"), batched=True)
        dataset_test = dataset["test"].map(lambda examples: preprocess_function(args, examples, tokenizer, "test"), batched=True)


    elif args.task == "gsmsymb":
        if args.expiter==2:
            dataset = load_from_disk("../datasets/data_gsm8k")
            dataset.save_to_disk("../datasets/data_gsm8k_escape")
        dataset_gsm8k = load_from_disk("../datasets/data_gsm8k_escape")

        filtered_dataset = remove_matching_questions(dataset_gsm8k, questions_to_remove)
        filtered_dataset.save_to_disk("../datasets/data_gsm8k_escape")
        # train with both train and test set
        dataset_train_train = dataset_gsm8k["train"].map(lambda examples: preprocess_function(args, examples, tokenizer, "train"), batched=True)
        dataset_train_test = dataset_gsm8k["test"].map(lambda examples: preprocess_function(args, examples, tokenizer, "test"), batched=True)

        dataset_train = concatenate_datasets([dataset_train_train, dataset_train_test])
        dataset_test = load_dataset("json", data_files="../datasets/gsm_symbolic/generated_data/GSM_symbolic.jsonl", split="train")
    ...
    # toy data load
    elif args.task == "toy_add_arithmetic":
        dataset_train = load_dataset("json", data_files="../datasets/toy/add_train.jsonl")["train"]
        dataset_test = load_dataset("json", data_files="../datasets/toy/add_test.jsonl")["train"]

        # Preprocess datasets
        dataset_train = dataset_train.map(lambda examples: preprocess_function(args, examples, tokenizer, "train"), batched=True)
        dataset_test = dataset_test.map(lambda examples: preprocess_function(args, examples, tokenizer, "test"), batched=True)

    elif args.task == "toy_sub_arithmetic":
        dataset_train = load_dataset("json", data_files="../datasets/toy/sub_train.jsonl")["train"]
        dataset_test = load_dataset("json", data_files="../datasets/toy/sub_test.jsonl")["train"]

        # Preprocess datasets
        dataset_train = dataset_train.map(lambda examples: preprocess_function(args, examples, tokenizer, "train"), batched=True)
        dataset_test = dataset_test.map(lambda examples: preprocess_function(args, examples, tokenizer, "test"), batched=True)

    elif args.task == "toy_counting":
        dataset_train = load_dataset("json", data_files="../datasets/toy/letter_count_train.jsonl")["train"]
        dataset_test = load_dataset("json", data_files="../datasets/toy/letter_count_test.jsonl")["train"]
        ...
        dataset_test = load_dataset("json", data_files="../datasets/data_arc_challenge/test.jsonl")["train"]
...
def get_dataloader(args, tokenizer, rank, world_size):
    ...
    if args.task == "gsm8k":
        ...
        dataset_gsm8k = load_from_disk("../datasets/data_gsm8k")
...
def preprocess_function(args, examples, tokenizer, split):
    if args.task == "logiqa":
        ...
        if split == "train":
            combined_texts = [f"{t}\nQ: {q}\nOptions:\n0.{o[0]}\n1.{o[1]}\n2.{o[2]}\n3.{o[3]}\nA: {a}" for t, q, o, a in zip(examples["text"], examples["question"], examples["options"], examples["answer"])]
        else:
     ...
     # ARC Challenge
        ...
        if split == "train":
            # combined_texts = [f"Q: {t} {q}\nA: {f}\n#### {a}" for t, q, f, a in zip(examples["body"], examples["question"], examples["formula"], examples["answer"])]
            combined_texts = [f"Q: {t} {q}\nA: #### {a}" for t, q, f, a in zip(examples["body"], examples["question"], examples["formula"], examples["answer"])]
        else:
    ...
    # preprocess of toy update
    elif args.task in ["toy_add_arithmetic", "toy_sub_arithmetic", "toy_counting"]:
        # Tokenize the input text and keep "question" and "answer"
        if split == "train":
            combined_texts = [f"Q: {q}\nA: {a}" for q, a in zip(examples["Q"], examples["A"])]
        else:
            combined_texts = [f"Q: {q}\nA: " for q in examples["Q"]]
        ...
        tokenized["question"] = examples["Q"]
        tokenized["answer"] = examples["A"]
